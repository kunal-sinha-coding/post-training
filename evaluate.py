...
def aggregate_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # Report partial and any-test completion rates beside pass@1 and aggregate test progress.
    metrics["partial_pass_fraction"] = sum(0 < int(result.get("passed_tests", 0)) < int(result.get("total_tests", 0)) for result in results) / total if total else 0.0
    metrics["partial_or_full_pass_fraction"] = sum(int(result.get("passed_tests", 0)) > 0 for result in results) / total if total else 0.0
